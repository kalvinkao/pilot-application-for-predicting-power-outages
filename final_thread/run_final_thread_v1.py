...
weatherData=sc.textFile("file:///home/w205/w205_final_project/final_thread/weather_history.csv")
#outageData=sc.textFile("file:///home/w205/w205_final_project/final_thread/outage_history.csv")
...
#RI_Weather = weatherRecords.map(lambda p: (p[5], p[6], p[14], p[15]))
...
#result_weatherOutage = sqlContext.sql('SELECT to_date(w.DTS) as DT, w.aveWindSpeed, w.maxTemp ,w.minTemp, case when o.DATETIME is null then 0 else 1 end as OutageIND  FROM RI_Weather w left outer join RI_Outages o on to_date(w.DTS) = to_date(concat(substr(DATETIME,7,4),"-",substr(DATETIME,1,2),"-",substr(DATETIME,4,2))) WHERE year(to_date(w.DTS))=2016 and month(to_date(w.DTS))=2  ORDER BY DT  LIMIT 100')
